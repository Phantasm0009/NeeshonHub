...
local NeeshonFolder = "Neeshon"
local ConfigurationFolder = NeeshonFolder.."/Configurations"
local ConfigurationExtension = ".rfld"



local NeeshonLibrary = {
	Flags = {},
	Theme = {
		Default = {
			TextFont = "Default", -- Default will use the various font faces used across Neeshon
		...
		Light = {
			TextFont = "Gotham",  -- Default will use the various font faces used across Neeshon
...
local Neeshon =  game:GetObjects("rbxassetid://11637506633")[1]

--studio
--function gethui() return Neeshon end local http_request = nil local syn = {protect_gui = false,request = false,}local http = nil function writefile(tt,t,ttt)end function isfolder(t)end function makefolder(t)end function isfile(r)end function readfile(t)end

if gethui then
	Neeshon.Parent = gethui()
elseif syn.protect_gui then 
	syn.protect_gui(Neeshon)
	Neeshon.Parent = CoreGui
elseif CoreGui:FindFirstChild("RobloxGui") then
	Neeshon.Parent = CoreGui:FindFirstChild("RobloxGui")
else
	Neeshon.Parent = CoreGui
end
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Neeshon.Name and Interface ~= Neeshon then
			Interface.Enabled = false
			Interface.Name = "Neeshon-Old"
		end
	end
else
...
local Notifications = Neeshon.Notifications

local SelectedTheme = NeeshonLibrary.Theme.Default

function ChangeTheme(ThemeName)
	SelectedTheme = NeeshonLibrary.Theme[ThemeName]
	for _, obj in ipairs(Neeshon:GetDescendants()) do
	...
	Neeshon.Main.BackgroundColor3 = SelectedTheme.Background
	Neeshon.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Neeshon.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Neeshon.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Neeshon.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Neeshon.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Neeshon.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor
...
local function LoadConfiguration(Configuration)
	...
	table.foreach(Data, function(FlagName, FlagValue)
		if NeeshonLibrary.Flags[FlagName] then
			spawn(function() 
				if NeeshonLibrary.Flags[FlagName].Type == "Colorpicker" then
					NeeshonLibrary.Flags[FlagName]:Set(UnpackColor(FlagValue))
				else
					if NeeshonLibrary.Flags[FlagName].CurrentValue or NeeshonLibrary.Flags[FlagName].CurrentKeybind or NeeshonLibrary.Flags[FlagName].CurrentOption ~= FlagValue then NeeshonLibrary.Flags[FlagName]:Set(FlagValue) end
				end    
			end)
		else
			NeeshonLibrary:Notify({Title = "Flag Error", Content = "Neeshon was unable to find '"..FlagName.. "'' in the current script"})
		end
	end)
end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i,v in pairs(NeeshonLibrary.Flags) do
...
function NeeshonLibrary:Notify(NotificationSettings)
	spawn(function()
		...
		if NotificationSettings.Actions then
			for _, Action in pairs(NotificationSettings.Actions) do
				...
				if SelectedTheme ~= NeeshonLibrary.Theme.Default then
				...
				NewAction.MouseButton1Click:Connect(function()
					...
					if not Success then
						print("Neeshon | Action: "..Action.Name.." Callback Error " ..tostring(Response))
					end
...
function Hide()
	Debounce = true
	NeeshonLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping RightShift", Duration = 7})
...
function NeeshonLibrary:CreateWindow(Settings)
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Neeshon Interface Suite"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "by Sirius"
	if Settings.LoadingTitle ~= "Neeshon Interface Suite" then
		LoadingFrame.Version.Text = "Neeshon UI"
	end
	...
	pcall(function()
		...
		if not isfolder(NeeshonFolder.."/".."Configuration Folders") then
	...
	if Settings.Discord then
		if not isfolder(NeeshonFolder.."/Discord Invites") then
			makefolder(NeeshonFolder.."/Discord Invites")
		end
		if not isfile(NeeshonFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(NeeshonFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Neeshon RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		else
	...
	if Settings.KeySystem then
		...
		if not isfolder(NeeshonFolder.."/Key System") then
			makefolder(NeeshonFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			...
			if not Success then
				print("Neeshon | "..Settings.KeySettings.Key.." Error " ..tostring(Response))
			end
		end
		...
		if isfile(NeeshonFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			if readfile(NeeshonFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) == Settings.KeySettings.Key then
				Passthrough = true
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				if KeyMain.Input.InputBox.Text == Settings.KeySettings.Key then
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(NeeshonFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, Settings.KeySettings.Key)
						end
						NeeshonLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully"})
					end
				else
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Neeshon | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function(enter)
				...
				if not Success then
					...
					print("Neeshon | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Input.Title.Text = InputSettings.Name
				...
				DropdownOption.Interact.MouseButton1Click:Connect(function()
					if Dropdown.Selected.Text ~= Option then
						...
						if not Success then
							...
							print("Neeshon | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						end
			...
			local function AddOptions(Options,Selected)
				...
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
						NeeshonLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
					end
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then  --Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Neeshon | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					NeeshonLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Neeshon | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Transparency = 0}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					NeeshonLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if Dragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Neeshon | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					NeeshonLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
			...
			for name,info in pairs(PromptSettings.Actions) do
				...
				Button.Interact.MouseButton1Up:Connect(function()
					if not clicked then
						...
						if not Success then
							ClosePrompt()
							print("Neeshon | "..info.Name.." Callback Error " ..tostring(Response))
						else
							ClosePrompt()
						end
					end
...
function NeeshonLibrary:Destroy()
	Neeshon:Destroy()
end
...
function NeeshonLibrary:LoadConfiguration()
	if CEnabled then
		pcall(function()
			if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
				LoadConfiguration(readfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension))
				NeeshonLibrary:Notify({Title = "Configuration Loaded", Content = "The configuration file for this script has been loaded from a previous session"})
			end
		end)
	end
end

return NeeshonLibrary
